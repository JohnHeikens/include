...
inline fp AddRelativisticVelocities(fp v1, fp v2, const fp& c)
{
	//Calculations
	fp Vf = abs(v1 + v2);
	v2 = -1 * v2;
...
//c: maximum speed objects may reach (the "speed of light")
//reference frame: v1
inline vec2 AddRelativisticVelocities(vec2 v1, vec2 v2, const fp& c)
{
...
inline void predictBehavior(const vec2& constantForce, vec2& speed, vec2& position, cfp& iterationCount)
{
	...
	position += speed * iterationCount + constantForce * calculateIterativeAddition(iterationCount);
