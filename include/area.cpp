...
calculation* area::GetCalculation(std::wstring equation, variable** result)
{
	...
	if (equation.size())
	{
		...
		if (i != std::wstring::npos)
		{
			int j = Find(equation, i + 1, L":", skip);
			//if
			c = new operatorIf();
		...
		if (i != std::wstring::npos)
		{
			...
			{
				ShowCompilerError(noclosingsquarebracket, equation);
			}
			if (j < equation.size() - 1)
			{
				ShowCompilerError(typedafterclosingbracket, equation);
			}
		...
		if (i != std::wstring::npos)
		{
			...
			if (j == std::wstring::npos)
			{
				ShowCompilerError(noclosingbrace, equation);
			}
			if (j < equation.size() - 1)
			{
				ShowCompilerError(typedafterclosingbracket, equation);
			...
			if (i == 0)//no functions, just specified to be calculated first
			{
				...
				if (!f)ShowCompilerError(couldnotfindfunction, equation);
				...
				{
					wstringcontainer arguments = split_string(betweenbraces, L",", skip);
		...
		ShowCompilerError(nooperator, equation);
		throw 0;
	check:
		...
		ShowCompilerError(calclengthzero, equation);
	}
}
